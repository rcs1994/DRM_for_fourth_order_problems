...
intx1,intx2,intx3 = np.split(int_col,3,axis=1)
bdx1,bdx2,bdx3 = np.split(bdry_col,3,axis=1)
nx1,nx2,nx3 = np.split(normal_vec,3,axis=1)

tintx1,tintx2,tintx3,tbdx1,tbdx2,tbdx3,tnx1,tnx2,tnx3 = tools.from_numpy_to_tensor([intx1,intx2,intx3,bdx1,bdx2,bdx3,nx1,nx2,nx3],[True,True,True,False,False,False,True,True,True],dtype=torch.float32)
...
loader = torch.utils.data.DataLoader([intx1,intx2,intx3],batch_size = 2000,shuffle = True)
...
def closure():
    ...
    for i, subquad in enumerate(loader):
        ...
        ttintx3 = Variable(subquad[2].float(), requires_grad=True)

        loss, loss_int_D2, loss_int, loss_neumann, loss_diri = pde.pdeloss(
            y, ttintx1, ttintx2,ttintx3, f, tbdx1, tbdx2, tbdx3, tnx1, tnx2, tnx3,
...
z_pts = np.linspace(0,1,200)

ms_x, ms_y, ms_z = np.meshgrid(x_pts,y_pts,z_pts)

x_pts = np.ravel(ms_x).reshape(-1,1)
t_pts = np.ravel(ms_y).reshape(-1,1)
z_pts = np.ravel(ms_z).reshape(-1,1)

collocations = np.concatenate([x_pts,t_pts,z_pts], axis=1)
