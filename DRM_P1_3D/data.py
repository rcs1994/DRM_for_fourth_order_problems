...
domain_data_z = uniform.rvs(size=N)

domain_data = np.array([domain_data_x,domain_data_y,domain_data_z]).T
...
def compute_normals(bdry_col, eps=1e-8):
    ...
    n1[mask] = -1.0;  n2[mask] =  0.0 ; n3[mask] =  0.0
    ...
    n1[mask] =  0.0;  n2[mask] = -1.0 ; n3[mask] =  0.0
    ...
    # botton edge z=0
    mask = np.isclose(z, 0.0, atol=eps)
    n1[mask] =  0.0;  n2[mask] =  0.0 ; n3[mask] = -1.0
    
    # top edge z=1
    mask = np.isclose(z, 1.0, atol=eps) 
    n1[mask] =  0.0;  n2[mask] =  0.0 ; n3[mask] =  1.0

    # reshape to column vectors
    return n1.reshape(-1,1), n2.reshape(-1,1), n3.reshape(-1,1)

bdry_col = generate_random_bdry(Nb)
n1_np, n2_np, n3_np = compute_normals(bdry_col)
normal_vec = np.hstack([n1_np, n2_np, n3_np])
