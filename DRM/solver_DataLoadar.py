...
class InteriorDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return self.x1[idx], self.x2[idx], self.f[idx]

# Create dataset and loader
batch_size = 500  # Try different values (e.g., 256, 1024) to tune performance
interior_dataset = InteriorDataset(tintx1, tintx2, f)
loader = torch.utils.data.DataLoader(interior_dataset, batch_size=batch_size, shuffle=True)

# Modified closure using DataLoader
def closure():
    tot_loss = 0.0
    for batch in loader:
        x1b, x2b, fb = batch
        x1b = x1b.requires_grad_(True)
        x2b = x2b.requires_grad_(True)

        optimizer.zero_grad()
        loss, _, _, _, _ = pde.pdeloss(y, x1b, x2b, fb, tbdx1, tbdx2, tnx1, tnx2, bdrydata_dirichlet, bdrydata_neumann, bw_dir, bw_neu)
        loss.backward()
        optimizer.step()
...
for epoch in range(400):
    ...
    if epoch % 100 == 0:
        ...
        # validation.plot_2D(y, name + f"y_plot/epoch{epoch}")
